...
const Edit_kladers = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // const response = await fetch(`/api/kladers/${Id}`);
        const response = await fetch(`/kladers.json`);
        ...
        console.error("Failed to fetch kladers data:", error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Kladers updated successfully");
    } catch (error) {
      console.error("Failed to update kladers data:", error);
      alert("Failed to update kladers");
    }
  };
  ...
  return (
    <div className="Add_kladers">
      <form onSubmit={handleSubmit}>
        <div className="new_title">{t("form.editKladers")}</div>
